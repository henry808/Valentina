...
class ArtGenerator:
    ...
    def generate_valentina_art(self, num_layers=4):
    ...
    def _generate_layer(self, canvas, color):
        ...
        for _ in range(num_shapes):
            shape_type = np.random.choice(["circle", "rectangle"], p=[0.7, 0.3])
    ...
    def _draw_shape(self, draw, shape_type, position, color):
        if shape_type == "circle":
            draw.ellipse(position, outline=tuple(color), width=3)
        else:  # "rectangle"
            draw.rectangle(position, outline=tuple(color), width=3)
...
if __name__ == "__main__":
    ...
    artwork = art_generator.generate_valentina_art(num_layers)
